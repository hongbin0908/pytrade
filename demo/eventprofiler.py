...
class Profiler:
	...
	def run(self, feed, useAdjustedCloseForReturns=True):
		...
		try:
			...
			for instrument in self.__rets:
				if self.__rets[instrument].getLength() > 0:
					#print instrument, self.__rets[instrument][-1]
					pass
			for instrument in  self.__events:
				#print instrument, 
				for event in self.__events[instrument]:
...
def build_plot(profilerResults):
	...
	plt.errorbar(x[eventT+1:], y[eventT+1:], std[eventT+1:], alpha=1, ecolor='#AAAAFF')
