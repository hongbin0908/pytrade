...
class ExtBrokeUpStrategy(strategy.BacktestingStrategy):
    ...
    def try_exit(self,instrument, datetime, barDs):
        ...
        if cur_close > entry_price * 1.10:
            if self.__stops[instrument] < cur_close * 0.96:
                self.__stops[instrument] = cur_close * 0.96
                position.cancelExit()
        ...
        if (self.__stops[instrument] < 0) :
            self.__stops[instrument] = entry_price * 0.96
            position.cancelExit()
            position.exit(stopPrice = self.__stops[instrument])
            ...
            elif self.__stops[instrument] < cur_close * 0.96 :
                pass
                #self.__stops[instrument] = cur_close * 0.99
                #position.cancelExit()
                #position.exit(stopPrice = self.__stops[instrument])
                #self.__extinfos[instrument]["pos_info"].append("%s set stop at %f" % (datetime, self.__stops[instrument]))
        return False
...
def main():
    ...
    feed = yahoofinance.build_feed(instruments, 2010,2010,".")
