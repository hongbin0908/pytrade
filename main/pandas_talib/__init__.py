...
def MACD(df, n_fast, n_slow, price='close'):
    ...
    """
    EMAfast = pd.Series(pd.ewma(df[price], span=n_fast, min_periods=n_fast - 1))
    ...
    MACDsign = pd.Series(pd.ewma(MACD, span=9, min_periods=8), name='ta_MACDsign_%d_%d' % (n_fast, n_slow))
