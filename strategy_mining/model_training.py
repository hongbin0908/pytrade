...
def get_predict_value():
   ...
   for s in file_list:
       ...
       volume.reverse()
       if len(open_prices) < 14:
           continue
       model.build_sample(numpy.array(open_prices[:-14]), numpy.array(high_prices[:-14]), numpy.array(low_prices[:-14]), numpy.array(close_prices[:-14]), numpy.array(adjust_close[:-14]), numpy.array(volume[:-14]), 14)
   model.post_process()
   model.model_process()
   num  = 0
   for s in file_list:
       ...
       result = model.result_predict(numpy.array(open_prices), numpy.array(high_prices), numpy.array(low_prices), numpy.array(close_prices),
           numpy.array(adjust_close), numpy.array(volume), 14)
       if result[0] > 1.12:
           num = num + 1
